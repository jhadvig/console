		...
		Object.keys(data).sort().forEach(k => {
			if (k === '.dockercfg' || k === ".dockerconfigjson") {
				...
				_.forEach(serverAddresses, (address, index) => {
					...
					dl.push(<h3 className="co-secret-section-header" key={`address-${index}`}>{address}</h3>);
				...
				if (decodedData.credsStore) {
					dl.push(<h3 className="co-secret-section-header" key={`credentials-store`}>Credentials Store</h3>);
					dl.push(<dd key="credentials-store-v"><pre className="co-pre-wrap">{showSecret ? decodedData.credsStore : '*****'}</pre></dd>);
				}
			} else {
				dl.push(<dt key={`${k}-k`}>{k}</dt>);
